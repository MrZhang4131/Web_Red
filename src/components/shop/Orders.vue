...
export default {
  ...
  methods: {
    ...
    getOrders() {
      this.$http({
        url: 'https://localhost:8002/Order?userid='+window.sessionStorage.getItem("id"),
        method: 'get'
      }).then(res => {
    ...
    //  console.log(this.orders[index].id);
     this.params.id = this.orders[index].id;
    ...
    addComment() {
      var _this = this;
      this.$http({
        url: 'https://localhost:8002/Order/comment',
        params: {
          goodsId:_this.params.id,
