  ...
  <div class="ShopCar">
    ...
    <div>
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item :to="{ path: '/Red/Shop/Goods' }"
    ...
    <div style="position: relative; height: 30px; line-height: 30px">
      ...
      >
        ...
        <el-button type="primary" plain @click="totalBuy()"
          >结算购物车</el-button
        >
      </div>
    </div>
    ...
    <el-table
      :data="
        shop_cart.slice((currentPage - 1) * pageSize, currentPage * pageSize)
      "
      stripe
    ...
    >
      ...
      <el-table-column prop="count" label="数量">
        <template v-slot="scope">
          <el-input-number
            v-model="shop_cart[scope.$index].count"
            :min="1"
            :max="10"
            label="购买数量"
            @change="
      ...
      <el-table-column prop="shopping_cart_id" label="操作">
        <template v-slot="scope">
          <el-button
            type="danger"
            plain
            @click="removeShopCartByIndex(scope.$index)"
            >删除</el-button
          >
    ...
    <!-- 分页 -->
    <div
      style="background-color: white; width: 100%; position: fixed; top: 90%"
    >
      <div class="block">
        <el-pagination
...
export default {
  data() {
    return {
      ...
      shop_cart: [
        {
          ...
          price: 18.5,
          picture:
            "https://tanhua-zxm.oss-cn-hangzhou.aliyuncs.com/%E5%B0%8F%E7%BA%A2%E4%B9%A6/%E5%95%86%E5%93%81/%E6%8E%A8%E8%8D%90/1/O1CN0160sQnm23r793wD3S8_%21%21267817308.jpg",
          count: 1,
          amount: 18.5,
          deleted: 0,
        },
      ...
      userInformation: {
        ...
        user_id: 4,
        user_picture:
          "http://192.168.123.53:8888/sports_shop_backend_war/users_picture/kzb68up.jpg",
        username: "冰镇生鲜",
        wallet: -2995,
      },
      // 当前页码
  ...
  methods: {
    ...
    getShopCart() {
      this.$http({
        url:
          "https://localhost:8002/ShoppingCar/shopcar?userid=" +
          window.sessionStorage.getItem("id"),
        method: "get",
      })
        .then((res) => {
          this.shop_cart = res.data;
        })
        .catch((res) => {
          console.log(res);
        });
    ...
    removeShopCartByIndex(index) {
      ...
      this.$http({
        url:
          "https://localhost:8002/ShoppingCar/remove?id=" +
          this.shop_cart[index].id,
        method: "put",
      })
        .then((res) => {
    ...
    handleChange(currentValue, oldValue, rowData) {
      ...
      if (currentValue - oldValue > 0) {
        this.$http({
          url: "https://localhost:8002/ShoppingCar/addCount",
          method: "post",
          params: {
            //键值对
            id: rowData.id,
            count: currentValue - oldValue,
          },
        })
          .then((res) => {
      ...
      // 减少购物车数量 http://localhost:8080/shoppingCart/reduceCount/{goodsId}
        ...
        this.$http({
          url: "https://localhost:8002/ShoppingCar/reduceCount",
          method: "post",
          params: {
            //键值对
            id: rowData.id,
    ...
    totalBuy() {
      ...
      console.log(mylist);
      this.$http({
        url: "https://localhost:8002/ShoppingCar/settlement",
        params: {
          id: mylist,
        },
        method: "post",
